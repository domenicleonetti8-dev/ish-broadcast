...
SCHEMA='eira2_bidirectional_transport_v2_2_1'
...
def worker_tick(r,root,s):
    ...
    if pf.exists():
        try:
            ...
            atomic(s/'runtime'/'stale_worker.json',{'schema':SCHEMA,'pid':pid,'cmdline':proc_cmdline(pid),'detected_utc':utc()})
    ...
    pf.write_text(str(p.pid)+'\n'); atomic(s/'runtime'/'worker.json',{'schema':SCHEMA,'state':'STARTED','pid':p.pid,'utc':utc()}); return {'state':'STARTED','pid':p.pid,'identity_verified':True}
...
def status(s,root):
    ...
    return {'schema':SCHEMA,'utc':utc(),'control_plane':str(s),'control_plane_on_live_drive':str(s).startswith(str(root)),'live_root':str(root),'storage_admitted':ok,'storage_evidence':ev,'cursor':c,'pending_blueprints':pending(s),'worker':worker,'inbox':{x:len(list((s/'inbox'/x).glob('*'))) for x in IN_LANES},'outbox':{x:len(list((s/'outbox'/x).glob('*'))) for x in OUT_LANES}}
def once(root,s):
