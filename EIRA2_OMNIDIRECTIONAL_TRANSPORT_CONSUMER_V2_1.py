...
def self_refresh(work:Path)->None:
    ...
    tmp=current.with_name(current.name+f'.refresh.{os.getpid()}'); tmp.write_bytes(rb); os.replace(tmp,current)
...
def watcher_authorize_retry(root:Path,request:dict[str,Any])->dict[str,Any]:
    ...
    for attempt in range(1,MAX_AUTH_RETRIES+1):
        try:
            ...
            if callable(fn):
                result=fn(request)
                if isinstance(result,dict) and result.get('authorized') is True: return result
...
def publish(base,work:Path,rid:str,receipt:dict[str,Any])->str: return base.publish(work,rid,receipt)


def run_superprobe(root:Path)->dict[str,Any]:
    probe=root/'tools'/'eira2_superprobe_engine.py'
    p=run([sys.executable,str(probe),'--root',str(root),'--json'],cwd=root,timeout=2400)
    combo=(p.stdout or '')+'\n'+(p.stderr or '')
    ...
    if not report_path.is_file(): raise RuntimeError('post_superprobe_report_missing')
    report=json.loads(report_path.read_text(encoding='utf-8'))
    if report.get('ok') is not True: raise RuntimeError('post_superprobe_report_not_ok')
    package=(report.get('package_truth') or {})
    if package and package.get('ok') is not True: raise RuntimeError('post_superprobe_package_not_ok')
    return {'ok':True,'evidence_fingerprint':report.get('evidence_bundle_fingerprint_sha256'),'schema':report.get('schema')}
...
def add_full_sources(base,root:Path,request:dict[str,Any],evidence:dict[str,Any])->None:
    ...
    if rows:
        evidence['full_sources']=rows; canonical=dict(evidence); canonical.pop('evidence_sha256',None)
...
def inspect_once(root:Path,parent_work:Path,state:Path,src:Path)->None:
    base=load_base(parent_work); raw=src.read_bytes(); digest=base.sha256_bytes(raw); req=json.loads(raw.decode()); rid=str(req.get('request_id') or '')
    done=state/f'{digest}.success.json'
    if done.exists(): return
    publish(base,parent_work,rid,{'schema':'eira2_transport_terminal_receipt_v2','request_id':rid,'operation':'inspect','transport_request_sha256':digest,'status':'STARTED','ok':None,'stage':'watcher_authorization','started_unix':time.time()})
    sync_repo(parent_work); base=load_base(parent_work)
    try:
        ...
        publish(base,parent_work,rid,{'schema':'eira2_transport_terminal_receipt_v2','request_id':rid,'operation':'inspect','transport_request_sha256':digest,'status':'RETRYING','ok':False,'builder_invoked':False,'error':f'{type(exc).__name__}:{exc}'[:2400],'retryable':True,'completed_unix':time.time()})
...
def deploy_worker(root:Path,runtime:Path,request_name:str)->int:
    control=runtime/'control_repo'; sync_repo(control); base=load_base(control); src=control/REQUEST_ROOT/request_name
    raw=src.read_bytes(); digest=base.sha256_bytes(raw); req=json.loads(raw.decode()); rid=str(req.get('request_id') or ''); op=str(req.get('operation') or '').casefold()
    ...
    try:
        auth=watcher_authorize_retry(root,req); dep=req.get('deployment') or {}; source=dep.get('source') or {}; target=dep.get('target') or {}
        ...
        payload=base.extract_payload(source_bytes,target_path); payload_sha=base.sha256_bytes(payload); live=root/target_path
        before=base.sha256_file(live) if live.is_file() else None
        if before==payload_sha:
            qualification=run_superprobe(root)
            evidence={'schema':'eira2_transport_deployment_evidence_v2','request_id':rid,'watcher_authorization':auth,'builder_invoked':False,'already_applied':True,'before_sha256':before,'after_sha256':before,'payload_sha256':payload_sha,'completion_sha256':before,'post_superprobe':qualification,'generated_unix':time.time()}
        else:
            evidence=base.deploy_request(root,work,req,auth); after=evidence.get('after_sha256')
        ...
        sync_repo(work); base=load_base(work); commit_sha=publish(base,work,rid,receipt); receipt['return_transport_commit']=commit_sha; atomic_json(runtime/'state'/f'{digest}.success.json',receipt); return 0
    except Exception as exc:
        try:
            sync_repo(work); base=load_base(work); publish(base,work,rid,{'schema':'eira2_transport_terminal_receipt_v2','request_id':rid,'operation':'deploy','transport_request_sha256':digest,'status':'RETRYING','ok':False,'builder_invoked':False,'error':f'{type(exc).__name__}:{exc}'[:3000],'retryable':True,'retry_after_seconds':WORKER_RETRY_SECONDS,'completed_unix':time.time()})
...
def main()->int:
    ap=argparse.ArgumentParser(); ap.add_argument('--root',required=True); ap.add_argument('--interval',type=float,default=2.0); ap.add_argument('--worker-request'); a=ap.parse_args(); root=Path(a.root).resolve()
