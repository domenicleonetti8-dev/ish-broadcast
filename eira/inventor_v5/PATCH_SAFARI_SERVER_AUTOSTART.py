...
HTML = r'''<!doctype html>
<html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1,viewport-fit=cover"><title>Eira Inventor Lab V5</title>
<style>
...
async function poll(){if(!job)return;try{let j=await api('/api/jobs/'+job);$('status').textContent=j.status+(j.stage?' - '+j.stage:'');$('report').textContent=JSON.stringify(j,null,2);if(j.status==='completed'){clearTimeout(timer);$('go').disabled=false;$('status').className='good';$('links').innerHTML=`<div class="row"><a rel="ar" href="${j.usdz_url}">Open / Download USDZ</a><a href="${j.glb_url}">Open GLB</a></div>`;return}if(j.status==='failed'){clearTimeout(timer);$('go').disabled=false;$('status').className='bad';return}timer=setTimeout(poll,1500)}catch(e){$('status').textContent=e.message;$('go').disabled=false}}
...
def _vision_provider():
    ...
    for mod, attr in candidates:
        try:
            m = __import__(f'extensions.eira_inventor_holographic_lab.{mod}', fromlist=[attr])
            p = getattr(m, attr)
...
def worker(job_id):
    with LOCK:
        j = JOBS[job_id]
        j['status'] = 'running'
        j['stage'] = 'vision/build/inspect/repair'
    try:
        ...
        with LOCK:
            j.update({'status':'failed','stage':'error','error':f'{type(e).__name__}:{e}','traceback':traceback.format_exc()[-5000:]})
...
class H(BaseHTTPRequestHandler):
    ...
    def log_message(self, fmt, *args):
        print('[InventorV5] ' + fmt % args, flush=True)
    def do_GET(self):
        p = urlparse(self.path).path
        if p == '/':
            return send(self, 200, HTML, 'text/html; charset=utf-8')
        if p == '/api/health':
            return send(self, 200, json_bytes({'ok':True,'version':'5.3.0','root':str(ROOT)}))
    ...
    def do_POST(self):
        ...
        try:
            ...
            for k, x in enumerate(obj.get('images') or []):
                name = safe_name(x.get('name') or f'image_{k}.jpg')
                f = src / name
                f.write_bytes(base64.b64decode(x.get('data_base64') or '', validate=True))
                imgs.append(str(f))
            ...
            return send(self,201,json_bytes(j))
        except Exception as e:
            return send(self,400,json_bytes({'error':f'{type(e).__name__}:{e}'}))

def main(host='0.0.0.0', port=8787):
    print(f'EIRA_INVENTOR_V5_SERVER http://{host}:{port}', flush=True)
...
def _port_open(host='127.0.0.1', port=8787):
    try:
...
def start_if_needed():
    ...
    env = os.environ.copy()
    env['EIRA_INVENTOR_AUTOSTART_CHILD'] = '1'
    subprocess.Popen([
        sys.executable, '-m', 'extensions.eira_inventor_holographic_lab.server',
        '--host', '0.0.0.0', '--port', '8787'
    ], cwd=str(root), env=env, stdout=log, stderr=log, start_new_session=True)
    return True
"""
(EXT / 'autostart.py').write_text(textwrap.dedent(autostart).lstrip(), encoding='utf-8')
py_compile.compile(str(EXT / 'autostart.py'), doraise=True)

site = ROOT / 'sitecustomize.py'
marker_a = '# BEGIN EIRA_INVENTOR_V5_AUTOSTART\n'
marker_b = '# END EIRA_INVENTOR_V5_AUTOSTART\n'
block = marker_a + """import os as _eira_os, sys as _eira_sys
try:
    if (_eira_os.path.basename(_eira_sys.argv[0]) == 'main.py' and
        _eira_os.environ.get('EIRA_INVENTOR_AUTOSTART_CHILD') != '1'):
        from extensions.eira_inventor_holographic_lab.autostart import start_if_needed as _eira_start_inventor
        _eira_start_inventor()
except Exception as _eira_inventor_boot_error:
    print('EIRA inventor web autostart warning:', _eira_inventor_boot_error)
""" + marker_b
existing = site.read_text(encoding='utf-8') if site.exists() else ''
if marker_a in existing and marker_b in existing:
    before = existing.split(marker_a,1)[0]
    after = existing.split(marker_b,1)[1]
...
obj['version'] = '5.3.0'
obj['web'] = {'module':'server','default_host':'0.0.0.0','default_port':8787,'path':'/','autostart_with_main':True}
m.write_text(json.dumps(obj, indent=2) + '\n')

print('SAFARI_SERVER_AUTOSTART_PATCH_PASS')
