...
def base(work:Path): return load(work/BASE_NAME,'eira2_transport_base_v23')
def watcher(root:Path): return load(root/'extensions'/'repair_watcher_ai'/'plugin.py','eira2_watcher_v23')
    ...
    try:
        ...
        if op=='inspect':
            ...
            if before==payload_sha:
                ev={'schema':'eira2_transport_deployment_evidence_v4','request_id':rid,'already_applied':True,'builder_invoked':False,'before_sha256':before,'after_sha256':before,'payload_sha256':payload_sha,'completion_sha256':before,'post_superprobe':run_superprobe(root),'generated_unix':time.time()}
            else:
        ...
        try:
            sync_repo(work); b=base(work); b.publish(work,rid,{'schema':'eira2_transport_terminal_receipt_v4','request_id':rid,'operation':op,'transport_request_sha256':digest,'status':'RETRYING','ok':False,'retryable':True,'retry_after_seconds':RETRY_SECONDS,'error':f'{type(exc).__name__}:{exc}'[:4000],'completed_unix':time.time()})
...
def main()->int:
    ...
    root=Path(a.root).resolve(); runtime=root/'eira_probe'/'transport_runtime_v2_3'
