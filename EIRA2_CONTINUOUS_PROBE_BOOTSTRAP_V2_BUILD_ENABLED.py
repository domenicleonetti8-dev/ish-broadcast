...
SCHEMA='eira2_continuous_probe_v2'; VERSION='2.0.0-build-enabled'; INTERVAL=8.0
...
'''
unit='''[Unit]\nDescription=EIRA Continuous Build and Evidence Probe V2\nAfter=network-online.target\nWants=network-online.target\nStartLimitIntervalSec=0\n\n[Service]\nType=simple\nWorkingDirectory=/media/domenicleonetti/easystore/EIRA/LIVE\nExecStart=/usr/bin/python3 /media/domenicleonetti/easystore/EIRA/LIVE/eira_probe/eira_continuous_probe.py\nRestart=always\nRestartSec=2\nKillMode=control-group\nTimeoutStopSec=10\n\n[Install]\nWantedBy=default.target\n'''
PROBE.parent.mkdir(parents=True,exist_ok=True); UNIT.parent.mkdir(parents=True,exist_ok=True)
compile(probe,str(PROBE),'exec')
backup=PROBE.with_name(PROBE.name+'.bak_v1')
if PROBE.is_file() and not backup.exists(): backup.write_bytes(PROBE.read_bytes())
tmp=PROBE.with_name(PROBE.name+'.tmp'); tmp.write_text(probe); os.replace(tmp,PROBE)
ut=UNIT.with_name(UNIT.name+'.tmp'); ut.write_text(unit); os.replace(ut,UNIT)
subprocess.run(['python3','-m','py_compile',str(PROBE)],check=True)
subprocess.run(['systemctl','--user','daemon-reload'],check=True)
subprocess.run(['systemctl','--user','restart','eira-continuous-probe.service'],check=True)
print('EIRA_CONTINUOUS_PROBE_V2_BUILD=STARTED')
