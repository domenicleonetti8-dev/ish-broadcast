...
SCHEMA='eira2_continuous_probe_v2'; VERSION='2.2.0-canonical-reseal'; INTERVAL=8.0
...
def reseal_plan(c,apply=False):
 ...
 trial=[dict(x) if isinstance(x,dict) else x for x in rows]
 trial_row=[x for x in trial if isinstance(x,dict) and x.get('path')==rel][0]
 trial_row['size']=new_size; trial_row['sha256']=new_sha
 after_tree=hashlib.sha256(canonical_bytes(trial)).hexdigest()
 ...
 plan.update({'backup':str(backup.relative_to(ROOT)),'manifest_after_sha256':sha(manifest)})
 return plan
def snapshot():
 rows=[]
 for rel in CORE:
