...
def _norm(v: str) -> str:
    return re.sub(r"\s+", " ", unicodedata.normalize("NFKC", str(v or "")).strip().lower())
def _stable_json(v: Any) -> str: return json.dumps(v,ensure_ascii=False,sort_keys=True,separators=(",",":"))
...
def canonical_seed_id(*,subject:str,claim:str,temporal_scope:str,epistemic_class:str)->str:
    return "seed_"+_sha(_stable_json({"subject":_norm(subject),"claim":_norm(claim),"temporal_scope":_norm(temporal_scope),"epistemic_class":_norm(epistemic_class)}))
def content_hash(seed: Mapping[str,Any])->str:
    return _sha(_stable_json({k:seed[k] for k in sorted(seed) if k not in {"source_refs","sources","relationships","seed_hash"}}))
def _req(seed:Mapping[str,Any],k:str)->str:
    v=str(seed.get(k,"")).strip()
    if not v: raise InvalidSeedError(f"missing_or_empty:{k}")
    return v
def validate_seed(seed:Mapping[str,Any])->dict[str,Any]:
    subject=_req(seed,"subject"); claim=_req(seed,"canonical_claim"); domain=_req(seed,"domain"); ep=_req(seed,"epistemic_class"); temporal=_req(seed,"temporal_scope"); truth=_req(seed,"truth_state")
    ...
    if not 0<=confidence<=1: raise InvalidSeedError("confidence_out_of_range")
    version=int(seed.get("version",1))
    if version<1: raise InvalidSeedError("invalid_version")
    out={"seed_id":canonical_seed_id(subject=subject,claim=claim,temporal_scope=temporal,epistemic_class=ep),"subject":subject,"canonical_claim":claim,"domain":domain,"epistemic_class":ep,"temporal_scope":temporal,"geographic_or_cosmic_scope":str(seed.get("geographic_or_cosmic_scope","unspecified")).strip() or "unspecified","truth_state":truth,"confidence":confidence,"version":version,"contradictions":list(seed.get("contradictions") or []),"metadata":dict(seed.get("metadata") or {})}
    out["seed_hash"]=content_hash(out); return out

class UniverseLibrary:
    """Durable archive storage only. Truth admission remains owned by Eira's evidence/Truth Gate authorities."""
    def __init__(self,db_path:str|Path):
        ...
        self.db.execute("PRAGMA foreign_keys=ON"); self.db.execute("PRAGMA journal_mode=WAL"); self.db.execute("PRAGMA synchronous=NORMAL"); self._init_schema()
    ...
    def _init_schema(self):
        ...
        CREATE TABLE IF NOT EXISTS seeds(seed_id TEXT PRIMARY KEY,seed_hash TEXT NOT NULL,subject TEXT NOT NULL,normalized_subject TEXT NOT NULL,canonical_claim TEXT NOT NULL,normalized_claim TEXT NOT NULL,domain TEXT NOT NULL,epistemic_class TEXT NOT NULL,temporal_scope TEXT NOT NULL,scope TEXT NOT NULL,truth_state TEXT NOT NULL,confidence REAL NOT NULL CHECK(confidence>=0 AND confidence<=1),version INTEGER NOT NULL CHECK(version>=1),contradictions_json TEXT NOT NULL,metadata_json TEXT NOT NULL,created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,updated_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,UNIQUE(normalized_subject,normalized_claim,temporal_scope,epistemic_class));
        CREATE TABLE IF NOT EXISTS sources(source_id TEXT PRIMARY KEY,source_uri TEXT NOT NULL,source_title TEXT NOT NULL DEFAULT '',publisher TEXT NOT NULL DEFAULT '',source_date TEXT NOT NULL DEFAULT '',retrieved_at TEXT NOT NULL,source_hash TEXT NOT NULL,source_kind TEXT NOT NULL DEFAULT 'unknown',metadata_json TEXT NOT NULL DEFAULT '{}',UNIQUE(source_hash,source_uri));
    ...
    def _source(self,s:Mapping[str,Any]):
        ...
        sid="src_"+_sha(_stable_json({"uri":uri,"hash":sh})); return sid,{"source_id":sid,"source_uri":uri,"source_title":str(s.get("source_title","")).strip(),"publisher":str(s.get("publisher","")).strip(),"source_date":str(s.get("source_date","")).strip(),"retrieved_at":retrieved,"source_hash":sh,"source_kind":str(s.get("source_kind","unknown")).strip() or "unknown","metadata_json":_stable_json(dict(s.get("metadata") or {}))}
    def archive_seed(self,seed:Mapping[str,Any],sources:Sequence[Mapping[str,Any]]=()):
        ...
        try:
            ...
            if ex is None:
                ...
                immutable=(ex["normalized_subject"],ex["normalized_claim"],ex["temporal_scope"],ex["epistemic_class"]); proposed=(_norm(n["subject"]),_norm(n["canonical_claim"]),n["temporal_scope"],n["epistemic_class"])
                if immutable!=proposed: raise DuplicateConflictError("canonical_seed_identity_conflict")
            ...
            for s in sources:
                source_id,row=self._source(s); self.db.execute("INSERT OR IGNORE INTO sources(source_id,source_uri,source_title,publisher,source_date,retrieved_at,source_hash,source_kind,metadata_json) VALUES(:source_id,:source_uri,:source_title,:publisher,:source_date,:retrieved_at,:source_hash,:source_kind,:metadata_json)",row)
                before=self.db.total_changes; self.db.execute("INSERT OR IGNORE INTO seed_sources(seed_id,source_id,relation) VALUES(?,?,?)",(sid,source_id,str(s.get("relation","supports")).strip() or "supports")); merged+=int(self.db.total_changes>before)
    ...
    def search(self,query:str,*,limit:int=25,truth_state:str|None=None):
        ...
        params.append(limit); return [dict(r) for r in self.db.execute(f"SELECT seed_id,subject,canonical_claim,domain,epistemic_class,temporal_scope,scope,truth_state,confidence,version FROM seeds WHERE (normalized_subject LIKE ? OR normalized_claim LIKE ? OR lower(domain) LIKE ?){extra} ORDER BY confidence DESC,seed_id LIMIT ?",params)]
...
def self_test_25x2():
    ...
    for r in (1,2):
        with tempfile.TemporaryDirectory(prefix=f"eira_universe_library_r{r}_") as td:
            ...
            with UniverseLibrary(db) as lib:
                ...
                checks += [(f"r{r}_01_schema",lib.schema_version()==3),(f"r{r}_02_db",db.exists()),(f"r{r}_03_insert",x.inserted),(f"r{r}_04_id_prefix",sid.startswith("seed_")),(f"r{r}_05_id_len",len(sid)==69),(f"r{r}_06_seed_count",lib.stats()["seeds"]==1),(f"r{r}_07_source_count",lib.stats()["sources"]==1),(f"r{r}_08_link_count",lib.stats()["seed_source_links"]==1)]
                ...
                z=lib.archive_seed(a,[_sample_source("https://example.invalid/source2","def")]); checks.append((f"r{r}_12_provenance_merge",z.merged_provenance==1 and lib.stats()["sources"]==2)); got=lib.get_seed(sid)
                checks += [(f"r{r}_13_lookup",got and got["subject"]=="Earth"),(f"r{r}_14_sources",got and len(got["sources"])==2),(f"r{r}_15_search",bool(lib.search("orbits"))),(f"r{r}_16_truth_filter",len(lib.search("Earth",truth_state="accepted"))==1),(f"r{r}_17_export",json.loads(lib.export_seed(sid))["seed_id"]==sid),(f"r{r}_18_deterministic",canonical_seed_id(subject=" Earth ",claim="Earth  orbits the Sun",temporal_scope="current",epistemic_class="fact")==sid)]
                rb=lib.archive_seed(_sample_seed("Sun","The Sun is a star"),[_sample_source("https://example.invalid/sun","sun")]); checks.append((f"r{r}_19_second_seed",lib.stats()["seeds"]==2)); checks.append((f"r{r}_20_edge",lib.add_edge(sid,"orbits",rb.seed_id) and lib.stats()["edges"]==1)); ok=False
    ...
    failed=[n for n,ok in checks if not ok]; return {"schema":"eira2_universe_library_qualification_v3","distinct_tests":25,"rounds":2,"clean_passes":sum(1 for _,ok in checks if ok),"total":len(checks),"failed":failed,"pass":len(checks)==50 and not failed}
'''
