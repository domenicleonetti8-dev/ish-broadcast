...
SCHEMA='eira2_bidirectional_transport_v2_2_2'
...
def mirror_new(r,s):
    ...
    for rel in changed_inbound(r,old,new):
        ...
        shutil.copy2(src,s/'inbox'/lane/src.name); counts[lane]+=1
...
def valid_worker_pid(pid):
    cmd=proc_cmdline(pid)
    return bool(cmd and 'eira2-transport-v2' in cmd and 'blueprint-once' in cmd)
def worker_tick(root,s):
    pf=s/'runtime'/'worker.pid'
    if pf.exists():
        try:
            pid=int(pf.read_text().strip()); os.kill(pid,0)
            if valid_worker_pid(pid): return {'state':'RUNNING','pid':pid,'identity_verified':True}
            atomic(s/'runtime'/'stale_worker.json',{'schema':SCHEMA,'pid':pid,'cmdline':proc_cmdline(pid),'detected_utc':utc()}); pf.unlink(missing_ok=True)
    ...
    pf.write_text(str(p.pid)+'\n'); atomic(s/'runtime'/'worker.json',{'schema':SCHEMA,'state':'STARTED','pid':p.pid,'utc':utc()})
    return {'state':'STARTED','pid':p.pid,'identity_verified':True}
def blueprint_once(root,s):
    transport_repo=sync_repo(s); consumer_path=transport_repo/CONSUMER
    spec=importlib.util.spec_from_file_location('eira2_bp_exact',consumer_path); mod=importlib.util.module_from_spec(spec); spec.loader.exec_module(mod)
    work=s/'blueprint-work'; mod.sync_repo(work)
    rows=[]
    for src in sorted((s/'inbox'/'blueprints').glob('*.json')):
        try:
            result=mod.process_blueprint(root,work,src,s/'blueprint-state')
            rows.append(result)
            mod.sync_repo(work)
            dst=s/'archive'/f'blueprints__{src.name}'
            if dst.exists(): dst.unlink()
            src.rename(dst)
        except Exception as exc:
            rows.append({'status':'CONSUMER_ERROR','path':src.name,'error':f'{type(exc).__name__}:{exc}'})
            mod.sync_repo(work)
    (s/'runtime'/'worker.pid').unlink(missing_ok=True)
    atomic(s/'runtime'/'last_blueprint_run.json',{'schema':SCHEMA,'scope':'cursor_selected_inbox_only','utc':utc(),'results':rows})
    return 0
def status(s,root):
    ok,ev=admission(root); c=cursor_path(s).read_text().strip() if cursor_path(s).exists() else None
    pf=s/'runtime'/'worker.pid'; worker=None
    if pf.exists():
        try:
            pid=int(pf.read_text().strip()); worker={'pid':pid,'identity_verified':valid_worker_pid(pid),'cmdline':proc_cmdline(pid)}
        except Exception: worker={'identity_verified':False}
    return {'schema':SCHEMA,'utc':utc(),'control_plane':str(s),'control_plane_on_live_drive':str(s).startswith(str(root)),'live_root':str(root),'storage_admitted':ok,'storage_evidence':ev,'cursor':c,'pending_blueprints':pending(s),'worker':worker,'blueprint_execution_scope':'cursor_selected_inbox_only','inbox':{x:len(list((s/'inbox'/x).glob('*'))) for x in IN_LANES},'outbox':{x:len(list((s/'outbox'/x).glob('*'))) for x in OUT_LANES}}
def once(root,s):
    dirs(s); r=sync_repo(s); incoming=mirror_new(r,s); worker=worker_tick(root,s); r=sync_repo(s); outgoing=publish_out(r,s); st=status(s,root)
    h={'schema':SCHEMA,'utc':utc(),'incoming':incoming,'worker':worker,'outgoing':outgoing,'status':st}; atomic(s/'runtime'/'heartbeat.json',h); return h
def main():
