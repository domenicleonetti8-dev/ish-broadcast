...
SCHEMA='eira2_orin_build_probe_v3'; VERSION='3.2-resilient-reseal'; POLL=3.0; UPGRADE_DIR=ROOT/'eira_probe'/'orin_build_probe_versions'; UPGRADE_REQUEST=ROOT/'eira_probe'/'orin_build_probe_supervisor_v3'/'upgrade_request.json'
...
MANIFEST=ROOT/'eira2-package-manifest.json'; PUBLIC_LIBRARY='eira2/evidence/universe_public_library.py'
...
def reseal_public_library_manifest(cmd):
 ...
 target=ROOT/path
 old_size=int(cmd.get('expected_old_size')); old_sha=str(cmd.get('expected_old_sha256') or '').lower(); new_size=int(cmd.get('expected_live_size')); new_sha=str(cmd.get('expected_live_sha256') or '').lower()
 if not MANIFEST.is_file() or not target.is_file(): raise RuntimeError('manifest_or_target_missing')
 live_size=target.stat().st_size; live_sha=sha_file(target)
 if live_size!=new_size or live_sha!=new_sha: raise RuntimeError('live_library_evidence_mismatch')
 ...
 before_tree=str(manifest.get('package_tree_sha256') or '')
 row['size']=new_size; row['sha256']=new_sha; after_tree=tree_hash(files); manifest['package_tree_sha256']=after_tree
...
def inspect(cmd):
 ...
 if op=='health': return {'root':str(ROOT),'state':str(STATE),'repo':str(REPO),'pid':os.getpid(),'version':VERSION,'capabilities':['read','list','search','compile','health','deploy','upgrade_probe','isolate_legacy_v1','reseal_public_library_manifest']}
