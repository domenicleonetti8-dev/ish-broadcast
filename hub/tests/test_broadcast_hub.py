...
from broadcast_hub import (  # noqa: E402
    AudioNode,
    BroadcastHub,
    LoopbackSupervisor,
    choose_input,
...
def config_for(directory: Path, **updates: Any) -> dict[str, Any]:
    value = {
        ...
        "status_path": str(directory / "hub.json"),
    }
    value.update(updates)
    return validate_config(value)


def adapter(address: str, path: str) -> AdapterState:
    return AdapterState(path, address, "old-name", False, False, False)
...
class ConfigurationTests(unittest.TestCase):
    ...
    def test_configured_input_is_selected(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual(
                choose_input(config, [source(), source("AA:BB:CC:DD:EE:02")]).address,
                "AA:BB:CC:DD:EE:02",
            )
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            atomic_write_json(
                config["bluez_status_path"],
                {
                    ...
                    "controller_discoverable": True,
                    "speakers": [
                        {"address": first.address, "connected": True},
                        {"address": second.address, "connected": True},
                    ],
                },
            )
            ...
            factory = FakeProcessFactory()
            supervisor = LoopbackSupervisor(config, factory)
            hub = BroadcastHub(config, graph, supervisor, config["status_path"])

            status = hub.reconcile_once(now=0.0)
            self.assertTrue(status["runtime_ready"])
            self.assertEqual(status["physical_audio_proof"], "not-recorded")
            first_process = supervisor.records[first.address].process
            second_process = supervisor.records[second.address].process

            status = hub.reconcile_once(now=1.0)
