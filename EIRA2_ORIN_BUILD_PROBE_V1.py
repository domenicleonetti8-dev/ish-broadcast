...
STATE = ROOT / "eira_probe" / "orin_build_probe_v1"
REPO = STATE / "repo"
...
SCHEMA = "eira2_orin_build_probe_v1"
POLL = 3.0
...
def publish(rid: str, receipt: dict[str, Any]) -> str:
    sync()
    rel = OUTBOX / f"{rid}.json"
    p = REPO / rel
    atomic_json(p, receipt)
    run(["git","add",rel.as_posix()], REPO)
    if run(["git","diff","--cached","--quiet"], REPO, check=False).returncode != 0:
        run(["git","-c","user.name=EIRA Orin Build Probe",
             "-c","user.email=eira-build-probe@localhost",
             "commit","--quiet","-m",f"Build probe receipt {rid}"], REPO)
        run(["git","pull","--rebase","--quiet","origin","master"], REPO)
        run(["git","push","--quiet","origin","HEAD:master"], REPO)
    return run(["git","rev-parse","HEAD"], REPO).stdout.strip()
...
def git_blob(commit: str, repo_path: str) -> bytes:
    ...
    p = subprocess.run(["git","-C",str(REPO),"show",f"{commit}:{rel.as_posix()}"],
                       capture_output=True, timeout=180, check=False)
    if p.returncode:
        raise RuntimeError("source_blob_missing:" + p.stderr[-1600:].decode(errors="replace"))
    return p.stdout

def backup(rid: str, target: Path) -> str | None:
    if not target.is_file():
        return None
    rel = target.relative_to(ROOT)
    dst = STATE / "backups" / rid / rel
...
def deploy(rid: str, cmd: dict[str, Any]) -> dict[str, Any]:
    ...
    target = safe_rel(str(cmd.get("target_path") or ""))
    raw = git_blob(str(src.get("commit") or ""), str(src.get("path") or ""))
    expected = str(src.get("sha256") or "").lower()
    if expected and sha_bytes(raw) != expected:
        raise RuntimeError("source_sha256_mismatch")
    if target.suffix == ".py":
        compile(raw.decode("utf-8"), str(target), "exec")
    before = target.read_bytes() if target.is_file() else None
    bak = backup(rid, target)
    target.parent.mkdir(parents=True, exist_ok=True)
    tmp = target.with_name(target.name + ".build_probe_tmp")
    tmp.write_bytes(raw)
    ...
    return {
        ...
        "before_sha256": sha_bytes(before) if before is not None else None,
        "after_sha256": after,
        "payload_sha256": sha_bytes(raw),
        "backup": bak,
        "compile_ok": True if target.suffix == ".py" else None,
    }

def inspect(cmd: dict[str, Any]) -> dict[str, Any]:
    op = str(cmd.get("op") or "")
    if op == "read":
        p = safe_rel(str(cmd.get("path") or ""))
        b = p.read_bytes()
        lim = min(max(int(cmd.get("max_bytes", 500000)), 1), 2000000)
        return {"path": str(p.relative_to(ROOT)), "bytes": len(b), "sha256": sha_bytes(b),
                "text": b[:lim].decode("utf-8","replace"), "truncated": len(b) > lim}
    if op == "list":
        p = safe_rel(str(cmd.get("path") or "."))
        lim = min(max(int(cmd.get("limit",500)),1),5000)
        return {"path": str(p.relative_to(ROOT)) if p != ROOT else ".",
                "items":[{"name":x.name,"type":"dir" if x.is_dir() else "file",
                          "size":x.stat().st_size if x.is_file() else None}
                         for x in sorted(p.iterdir())[:lim]]}
    if op == "search":
        term = str(cmd.get("term") or "")
        roots = [safe_rel(x) for x in (cmd.get("roots") or ["eira2","extensions","tools"])]
        lim = min(max(int(cmd.get("limit",100)),1),500)
        hits=[]
        for base in roots:
            ...
            for p in base.rglob("*"):
                ...
                if term in text:
                    hits.append({"path":str(p.relative_to(ROOT)),"sha256":sha_file(p),"bytes":p.stat().st_size})
        return {"term":term,"hits":hits}
    if op == "compile":
        p=safe_rel(str(cmd.get("path") or ""))
        q=run(["python3","-m","py_compile",str(p)],timeout=60,check=False)
        return {"path":str(p.relative_to(ROOT)),"ok":q.returncode==0,
                "returncode":q.returncode,"stderr":q.stderr[-2000:]}
    if op == "health":
        return {"root":str(ROOT),"state":str(STATE),"repo":str(REPO),"pid":os.getpid()}
...
def execute(cmd: dict[str, Any]) -> dict[str, Any]:
    ...
    if op == "deploy":
        base.update(ok=True, result=deploy(rid,cmd))
    else:
        base.update(ok=True, result=inspect(cmd))
    return base
    ...
    while True:
        try:
            ...
            atomic_json(STATE/"heartbeat.json",
                        {"schema":SCHEMA,"ok":True,"phase":"synced","head":head,"pid":os.getpid(),"unix":time.time()})
            inbox = REPO / INBOX
            inbox.mkdir(parents=True, exist_ok=True)
            for p in sorted(inbox.glob("*.json")):
                raw=p.read_bytes(); digest=sha_bytes(raw)
                marker=executed/f"{digest}.json"
                if marker.exists(): continue
                try:
                    cmd=json.loads(raw.decode("utf-8"))
                    rec=execute(cmd)
                except Exception as e:
                    rec={"schema":SCHEMA,"id":p.stem,"ok":False,
                         "error":f"{type(e).__name__}:{e}"[:4000],"unix":time.time(),"root":str(ROOT)}
                rec["command_sha256"]=digest
                atomic_json(marker,rec)
                rec["return_commit"]=publish(str(rec.get("id") or p.stem),rec)
            time.sleep(max(1.0,interval))
        except Exception as e:
            atomic_json(STATE/"heartbeat.json",
                        {"schema":SCHEMA,"ok":False,"phase":"error","error":f"{type(e).__name__}:{e}"[:3000],
                         "pid":os.getpid(),"unix":time.time()})
            time.sleep(max(2.0,interval))

def main():
    ap=argparse.ArgumentParser()
    ap.add_argument("--interval",type=float,default=POLL)
    ap.add_argument("--once",action="store_true")
    args=ap.parse_args()
    if args.once:
        head=sync()
        print(json.dumps({"schema":SCHEMA,"ok":True,"head":head,"root":str(ROOT)},indent=2))
        return
    loop(args.interval)
