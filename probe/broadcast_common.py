#!/usr/bin/env python3
"""Shared configuration and state helpers for the Broadcast probe."""
...
DEFAULT_CONFIG: dict[str, Any] = {
    ...
    "bluez_status_path": "/run/broadcast-probe/bluez-status.json",
    "status_path": "",
}


class ConfigError(ValueError):
    """Raised when a probe configuration would violate a hard requirement."""
...
def validate_config(raw: Mapping[str, Any]) -> dict[str, Any]:
    ...
    config["status_freshness_seconds"] = _number(
        config, "status_freshness_seconds", 1.0, 60.0
    )
