# Broadcast Bluetooth probe

The probe is the component that makes **broadcast** appear in the iPhone's own
...
Eira is not part of this endpoint or audio path. Bluetooth discovery and audio
remain local to the portable unit. The installer and services refuse to run on
hostname `eira`, preventing the home host from being mistaken for the probe
that travels with Dom.
...
No Mac, Xcode, TestFlight, jailbreak, or iSH runtime is required for the probe.
...
`/etc/broadcast-probe/config.json` on later runs.
...
`/etc/broadcast-probe/config.json`:
...
systemctl --user restart broadcast-probe.service
```
...
The connection gate requires all of the following:

- BlueZ readback proves the exact alias, powered/pairable/discoverable state,
  and registered local Audio Sink UUID;
- the paired inbound device is connected, service-resolved, and publishes the
  Audio Source UUID;
- a matching `bluez_input.*` node exists;
- every connected speaker is paired, trusted, connected, service-resolved,
  publishes the Audio Sink UUID, has a matching `bluez_output.*` node, and has
  its own live loopback process;
- `runtime_ready` is true; `end_to_end_streaming` becomes true only when the
  matching PipeWire nodes report `running`.
...
systemctl --user status broadcast-probe.service
broadcast-status
...
`/run/user/UID/broadcast-probe/status.json`; the system service writes controller
and reconnect state to `/run/broadcast-probe/bluez-status.json`.
...
The suite uses controlled BlueZ controllers, devices, PipeWire nodes, and
loopback processes to verify exact naming, role wiring, readback requirements,
stale-evidence rejection, allowlists, controller binding, the 10-device cap,
reconnect cooldowns, per-speaker delays, dynamic join/leave, installer
idempotence, and failure isolation. Test doubles are never presented as radio
or audible proof.
