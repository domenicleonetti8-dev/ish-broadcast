...
SCHEMA='eira2_orin_build_probe_v3'; POLL=3.0; UPGRADE_DIR=ROOT/'eira_probe'/'orin_build_probe_versions'; UPGRADE_REQUEST=ROOT/'eira_probe'/'orin_build_probe_supervisor_v3'/'upgrade_request.json'
...
def hb(phase,ok=True,**kw): atom(STATE/'heartbeat.json',{'schema':SCHEMA,'ok':ok,'phase':phase,'pid':os.getpid(),'unix':time.time(),'version':'3.0-resilient',**kw})
...
def inspect(cmd):
 ...
 if op=='health': return {'root':str(ROOT),'state':str(STATE),'repo':str(REPO),'pid':os.getpid(),'version':'3.0-resilient'}
...
def main():
 ...
 if a.once: print(json.dumps({'schema':SCHEMA,'ok':True,'head':sync(),'root':str(ROOT)},indent=2)); return
 loop(a.interval)
