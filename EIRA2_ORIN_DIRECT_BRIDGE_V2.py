...
def request(rid):
    ...
    if req.get('request_id')!=rid or req.get('operation')!='deploy': raise RuntimeError('request_invalid')
    ...
    out=json.loads(receipt.read_text()); out.update({'bridge':'EIRA2_ORIN_DIRECT_BRIDGE_V2','request_id':rid,'payload_sha256':sha(payload),'target_path':tp})
    local=ROOT/'eira_probe/orin_direct_bridge_v2/receipts'/f'{rid}.json'; atomic(local,(json.dumps(out,indent=2,sort_keys=True)+'\n').encode())
