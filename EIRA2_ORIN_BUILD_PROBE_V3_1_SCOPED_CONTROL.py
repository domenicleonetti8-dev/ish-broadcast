...
SCHEMA='eira2_orin_build_probe_v3'; VERSION='3.1-scoped-control'; POLL=3.0; UPGRADE_DIR=ROOT/'eira_probe'/'orin_build_probe_versions'; UPGRADE_REQUEST=ROOT/'eira_probe'/'orin_build_probe_supervisor_v3'/'upgrade_request.json'
...
def stage_upgrade(rid,cmd):
 ...
 atom(UPGRADE_REQUEST,{'schema':'eira2_orin_build_probe_upgrade_v1','id':rid,'candidate':str(cand.relative_to(ROOT)),'sha256':digest,'requested_unix':time.time()}); return {'candidate':str(cand.relative_to(ROOT)),'sha256':digest,'handoff':'supervisor'}
def restart_continuous_probe():
 service='eira-continuous-probe.service'
 q=run(['systemctl','--user','restart',service],timeout=30,check=False)
 if q.returncode: raise RuntimeError('continuous_probe_restart_failed:'+(q.stderr or q.stdout)[-1200:])
 s=run(['systemctl','--user','is-active',service],timeout=15,check=False)
 if s.returncode or s.stdout.strip()!='active': raise RuntimeError('continuous_probe_not_active:'+(s.stderr or s.stdout)[-500:])
 return {'service':service,'active':True,'status':s.stdout.strip()}
def inspect(cmd):
 ...
 if op=='health': return {'root':str(ROOT),'state':str(STATE),'repo':str(REPO),'pid':os.getpid(),'version':VERSION}
 if op=='restart_continuous_probe': return restart_continuous_probe()
