...
def run_job(job,image_path,user_text,out_dir,vision=ollama_vision,blender="blender"):
    ...
    def stage(s,**kw): job.update(status=s,updated=time.time(),**kw); (out/"job.json").write_text(json.dumps(job,indent=2,default=str))
    try:
        ...
        stage("engineering_compiling"); compiled,scene=compile_assembly(assembly); (out/"compiled_ir.json").write_text(json.dumps(compiled,indent=2))
        stage("validating")
        ...
        stage("blender_running"); cp=subprocess.run([blender,"-b","--python",str(script)],capture_output=True,text=True,timeout=600)
        ...
        stage("completed",model_url=str(glb)); return job
