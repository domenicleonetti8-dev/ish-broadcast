...
import hashlib,json,os,queue,shutil,subprocess,threading,time,urllib.request

ROOT=Path('/media/domenicleonetti/easystore/EIRA/LIVE').resolve()
STATE=ROOT/'eira_probe/orin_worker_v3'
REPO=Path('/tmp/eira2_orin_worker_v3_repo')
...
SCHEMA='eira2_orin_worker_receipt_v3'
...
def fetch_cmd():
 url=f'{CURRENT}?t={time.time_ns()}'
 q=urllib.request.Request(url,headers={'User-Agent':'EIRA2-ORIN-WORKER/3','Cache-Control':'no-cache, no-store','Pragma':'no-cache'})
 with urllib.request.urlopen(q,timeout=15) as r:return json.loads(r.read().decode())

def heartbeat(**v):
 jwrite(STATE/'heartbeat.json',{'schema':'eira2_orin_worker_heartbeat_v3','pid':os.getpid(),'unix':time.time(),**v})
...
def publish_batch(items):
 ...
 if rr(['git','diff','--cached','--quiet'],REPO,120,False).returncode!=0:
  rr(['git','-c','user.name=EIRA Orin Worker','-c','user.email=eira-orin-worker@localhost','commit','--quiet','-m','Orin worker V3 tether update'],REPO,120)
...
def filesystem_probe():
 while not STOP.is_set():
  try:
   ...
   snap={'schema':'eira2_probe_filesystem_v1','unix':time.time(),'root':str(ROOT),'items':items[:500]}
   jwrite(STATE/'probes/filesystem.json',snap);enqueue(f'{TELE}/filesystem_latest.json',snap)
...
def runtime_probe():
 while not STOP.is_set():
  try:
   q=rr(['ps','-eo','pid,ppid,stat,etime,cmd','--sort=pid'],timeout=20,check=False)
   lines=[x for x in q.stdout.splitlines() if 'EIRA/LIVE' in x or 'eira2_' in x or 'main.py' in x]
   snap={'schema':'eira2_probe_runtime_v1','unix':time.time(),'loadavg':Path('/proc/loadavg').read_text().strip() if Path('/proc/loadavg').exists() else None,'processes':lines[:300]}
   jwrite(STATE/'probes/runtime.json',snap);enqueue(f'{TELE}/runtime_latest.json',snap)
  ...
  try:
   ...
   for base in roots:
    ...
    for dirpath,dirnames,filenames in os.walk(base):
     ...
     for name in names:
      low=name.lower()
      if any(k in low for k in ('library','textbook','book','seed','chunk','corpus')):
       q=p/name
     ...
     if len(hits)>=500:break
    if len(hits)>=500:break
   snap={'schema':'eira2_probe_library_v1','unix':time.time(),'candidates':hits}
   jwrite(STATE/'probes/library.json',snap);enqueue(f'{TELE}/library_latest.json',snap)
...
def publisher():
 ...
 while not STOP.is_set():
  ...
  if pending:
   ...
   try:
    heartbeat(ok=True,phase='publishing_telemetry',queued=len(batch));commit=publish_batch(batch);heartbeat(ok=True,phase='idle',telemetry_commit=commit)
   except Exception as e:
    for rel,obj in batch:pending[str(rel)]=(rel,obj)
    heartbeat(ok=False,phase='publish_error',error=f'{type(e).__name__}:{e}',queued=len(pending))
  STOP.wait(5)
...
def command_loop():
 ...
 while not STOP.is_set():
  try:
   c=fetch_cmd();rid=str(c.get('id','')).strip()
   if rid:
    saved=executed/f'{rid}.json'
    if saved.exists():
     rec=json.loads(saved.read_text());enqueue(f'{OUT}/{rid}.json',rec)
    else:
     ...
     heartbeat(ok=True,phase='executed',id=rid)
  except Exception as e:heartbeat(ok=False,phase='command_error',error=f'{type(e).__name__}:{e}')
  STOP.wait(3)

def main():
 STATE.mkdir(parents=True,exist_ok=True);heartbeat(ok=True,phase='boot')
 threads=[threading.Thread(target=filesystem_probe,daemon=True,name='filesystem_probe'),threading.Thread(target=runtime_probe,daemon=True,name='runtime_probe'),threading.Thread(target=library_probe,daemon=True,name='library_probe'),threading.Thread(target=publisher,daemon=True,name='publisher')]
