...
def clone_repo(work: Path) -> None:
    ...
    if p.returncode:
        raise RuntimeError(f"git_clone_failed:{p.stderr[-1000:]}")
    p = run(["git", "checkout", "--quiet", "master"], cwd=work)
    if p.returncode:
        raise RuntimeError(f"git_checkout_failed:{p.stderr[-1000:]}")


def publish(work: Path, packet_id: str) -> None:
    p = run(["git", "add", str(RECEIPT_DIR / packet_id)], cwd=work)
    if p.returncode:
        raise RuntimeError(f"git_add_failed:{p.stderr[-1000:]}")
    ...
    p = run([
        ...
        "commit", "--quiet", "-m", f"Publish Builder receipt {packet_id}",
    ], cwd=work)
    if p.returncode:
        raise RuntimeError(f"git_commit_failed:{p.stderr[-1000:]}")
    p = run(["git", "push", "--quiet", "origin", "master"], cwd=work, timeout=300)
    if p.returncode:
        raise RuntimeError(f"git_push_failed:{p.stderr[-1000:]}")


def process_once(root: Path, work: Path) -> dict:
    clone_repo(work)
    remote = work / REMOTE_DIR
    remote.mkdir(parents=True, exist_ok=True)
    local_inbox = root / "eira_probe" / "transport_inbox" / "blueprints"
    done_dir = root / "eira_probe" / "blueprint_transport_done"
    runtime = root / "eira_probe" / "transport_runtime"
    lane = runtime / "eira2_blueprint_deployment_lane.py"
    if not lane.is_file():
        raise RuntimeError("blueprint_deployment_lane_missing")
    ...
    for src in sorted(remote.glob("*.json")):
        ...
        proc = run([sys.executable, str(lane), "--root", str(root), "--packet", str(local)], cwd=root)

        dest = work / RECEIPT_DIR / packet_id
        dest.mkdir(parents=True, exist_ok=True)
        lane_receipt = root / "eira_probe" / "blueprint_receipts" / f"{packet_id}.lane.json"
        builder_receipt = root / "eira_probe" / "blueprint_receipts" / f"{packet_id}.builder.json"
        ...
        transport_receipt = {
            "schema": "eira2_blueprint_transport_receipt_v1",
            "packet_id": packet_id,
...
def main() -> int:
    ...
    ap.add_argument("--work", default="/tmp/eira2_blueprint_transport")
    ...
    while True:
        try:
            ...
            print(json.dumps({"EIRA2_BLUEPRINT_TRANSPORT": "PASS", **result}, indent=2))
        except Exception as exc:
            print(json.dumps({"EIRA2_BLUEPRINT_TRANSPORT": "FAIL", "error": f"{type(exc).__name__}:{exc}"}, indent=2), file=sys.stderr)
            if not args.watch:
                return 1
        if not args.watch:
            return 0
