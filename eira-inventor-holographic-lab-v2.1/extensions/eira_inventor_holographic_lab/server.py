...
class Handler(BaseHTTPRequestHandler):
    server_version = "EIRA-InventorLab/2.3"
    ...
    def do_GET(self):
        ...
        try:
            ...
            if p.startswith("/api/jobs/"): return self.send_json(200, plugin.get_job(unquote(p.rsplit("/", 1)[-1])))
            if p.startswith("/api/inventions/"): return self.send_json(200, plugin.get_invention(unquote(p.rsplit("/", 1)[-1])))
    ...
    def do_POST(self):
        ...
        try:
            ...
            if p == "/api/chat":
                context = {
                    "invention_id": obj.get("invention_id"),
                    "selected_part": obj.get("selected_part") if isinstance(obj.get("selected_part"), dict) else None,
                }
                return self.send_json(200, plugin.chat(obj.get("message", ""), context))
