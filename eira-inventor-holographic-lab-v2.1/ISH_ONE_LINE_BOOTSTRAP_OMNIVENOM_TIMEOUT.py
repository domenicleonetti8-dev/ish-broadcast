...
import hashlib, json, os, shutil, signal, sys, tempfile, time, urllib.request
...
def fetch(rel):
    ...
    req=urllib.request.Request(BASE+rel,headers={'User-Agent':'EIRA-iSH-InventorLab-v2.1-omnivenom-timeout'})
...
try:
    ...
    if not contract.get('ok'):
        print('ENGINEERING3D_DIAGNOSTIC='+json.dumps(contract,default=str,sort_keys=True)); die('engineering3d contract unresolved')
    from extensions.omnivenom_mesh_ai.runtime import Omnivenom
    mesh=Omnivenom(live)
    bounded('OMNIVENOM_REFRESH',30,mesh.refresh)
    evidence=bounded('OMNIVENOM_CONTEXT',30,lambda: mesh.context(NODE_ID,depth=1,limit=80))
    if NODE_ID not in json.dumps(evidence,default=str):
        print('OMNIVENOM_EVIDENCE='+json.dumps(evidence,default=str)[:4000])
        die('OmniVenom did not discover node')
except BaseException as exc:
    print('INSTALL_FAILURE='+repr(exc))
    if target.exists(): shutil.rmtree(target,ignore_errors=True)
    if backup and backup.exists(): shutil.copytree(backup,target)
    raise
print('LIVE_INSTALL=PASS'); print('NODE_ID='+NODE_ID); print('LIVE='+str(live)); print('ENGINEERING3D_ENTRYPOINT='+str(contract.get('entrypoint'))); print('ENGINEERING3D_SHA256='+str(contract.get('provider_sha256'))); print('CAPABILITY_REQUEST='+str(contract.get('request_type'))+str(contract.get('request_signature'))); print('OMNIVENOM_DISCOVERY=PASS'); print('CORE_MAIN_PRESERVED=true'); print('START=cd '+str(live)+' && python3 -m extensions.eira_inventor_holographic_lab.server --host 127.0.0.1 --port 8787')