...
try:
    ...
    out={'schema':'eira2_boot_preflight_stage_profile_v1','ok':True,'mutates_live':False,'rows':rows,'service_count':len(runtime.supervisor._records),'topological_order':runtime.supervisor.topological_order()}
except Exception as e:
    out={'schema':'eira2_boot_preflight_stage_profile_v1','ok':False,'mutates_live':False,'rows':rows,'error':f'{type(e).__name__}:{e}'}
