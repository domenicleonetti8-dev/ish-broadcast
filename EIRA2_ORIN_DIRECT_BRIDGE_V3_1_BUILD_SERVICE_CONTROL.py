...
ALLOWED_USER_SERVICES={'eira-continuous-probe.service','eira2-orin-build-probe.service'}

def get(url, timeout=20):
    req=urllib.request.Request(url,headers={'User-Agent':'EIRA2-Orin-Direct-Bridge-V3.1','Cache-Control':'no-cache'})
...
def deploy(rid, req):
    ...
    out=json.loads(receipt.read_text()); out.update({'bridge':'EIRA2_ORIN_DIRECT_BRIDGE_V3_1','request_id':rid,'payload_sha256':sha(payload),'target_path':tp,'operation':'deploy'})
    return out
def restart_user_service(rid, req):
    ...
    return {'bridge':'EIRA2_ORIN_DIRECT_BRIDGE_V3_1','request_id':rid,'operation':'restart_user_service','service':svc,'before':before.stdout.strip(),'after':after.stdout.strip(),'ok':q.returncode==0}
def request(rid):
